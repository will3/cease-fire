...
export default class Server {
  ...
  public start() {
    this.io.on("connection", (socket) => {
      ...
      socket.on("join", (data) => {
        console.log(
          `player joined socket ${connection.socket.id} player ${data.playerId}`
        );
      ...
      socket.on("disconnect", () => {
        console.log(
          `player left socket ${connection.socket.id} player ${connection.playerId}`
        );
  ...
  private getClientState = (id: string): ClientState => {
    const components = _(this.runner.components)
      .filter((c) => c.isRemote)

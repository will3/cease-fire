...
import {
  ...
  randomQuaternion,
  randomSigned,
  randomUniformUnitVectors,
} from "../math";
import {
  Bounds,
  calcBounds,
  calcCenterFromVectors,
  divideCoords,
} from "../voxel/utils";
...
export default class ShipCutter extends Component {
    ...
    _(this.shipBody.chunk.map).forEach((line) => {
      ...
      for (let i = 0; i < planes.length; i++) {
        ...
        const distance = plane.distanceToPoint(
          line.coord.clone().add(new Vector3(0.5, 0.5, 0.5))
        );
  ...
  private createPieces(results: CutResult) {
    ...
    groups.forEach((group, index) => {
      ...
      piece.rotationSpeed = randomQuaternion(
        (rotationSpeedBase * 2) / rotationInertia,
        this.random
      );
      piece.velocity = dir.multiplyScalar(
        (random(0.5, 1, this.random) * 0.4) / mass
      );
    });
  }
  ...
  private randomCutPlane(bounds: Bounds) {
    ...
    const x =
      halfX + halfX * Math.pow(this.random(), 1.2) * randomSigned(this.random);

...
export default class ShipBody extends Component implements Hitable {
  ...
  public start() {
    ...
    this.chunkMesh.mesh.userData = {
      componentId: this.id,
      shipId:
        this.parentComponent == null ? undefined : this.parentComponent.id,
    };
  ...
  public createPiece(coords: Vector3[]) {
    ...
    piece.object.position.copy(
      this.chunkMesh.mesh.getWorldPosition(new Vector3())
    );
    piece.object.quaternion.copy(
      this.chunkMesh.mesh.getWorldQuaternion(new Quaternion())
    );
    ...
    for (const p of pattern) {
      ...
      if (this.chunk.inBound(dc.x, dc.y, dc.z)) {
        ...
        this.chunk.setColor(
          ...
          dc.y,
          dc.z,
          this.damageColor.clone().lerp(this.color, v)
        );
      }
    }
...
const buildShip = (chunk: Chunk, color: Color) => {
  ...
  function addWeapon(i: number) {
    voxels.push([i, 0, 1], [i, 0, 2], [i, 0, 5]);
  }

  function addCargo(i: number) {
    voxels.push([i, 0, 2], [i, 0, 5]);
  }
